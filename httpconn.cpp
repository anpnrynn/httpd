...
void HttpReq::processHttpMethod(char* mthd)
{
	...
	if( strcasecmp("POST",mthd) == 0 )
		method = HTTP_POST;
	else
	if( strcasecmp("HEAD",mthd) == 0 )
		method = HTTP_HEAD;
	else
		method = HTTP_UNKNOWN;
...
char* HttpReq::getTagBuffer(char *tag )
{
	...
	if( strcasecmp ("Accept",tag) == 0 )
		return accept;
	else
	if( strcasecmp ("Accept-Encoding",tag) == 0 )
		return acceptEnc;
	else
	if( strcasecmp ("Accept-Charset",tag) == 0 )
		return acceptChar;
	else
	if( strcasecmp ("Accept-Language",tag) == 0 )
		return acceptLang;
	else
	if( strcasecmp ("User-Agent",tag) == 0 )
		return userAgent;
	else
	if( strcasecmp ("Referer",tag) == 0 )
		return referer;
	else
	if( strcasecmp ("Cookie",tag) == 0 )
		return cookie;
	else
	if( strcasecmp ("Encoding",tag) == 0 )
		return encoding;
	else
	if( strcasecmp ("Content-Type",tag) == 0 )
		return contentType;
	else
	if( strcasecmp ("Date", tag) == 0 )
		return dateStr;
	else
	if( strcasecmp ("Transfer-Encoding", tag) == 0 )
		return transEnc;
	else
	if( strcasecmp ("If-Modified-Since", tag) == 0 )
		return ifModStr;
	else
	if( strcasecmp ("Content-Length",tag) == 0 )
		return contentLen;
	else
	if( strcasecmp ("Connection",tag) == 0 )
		return connection;
	else
	if( strcasecmp ("Keep-Alive",tag) == 0 )
		return keepAlive;
	else
		return NULL;
}
